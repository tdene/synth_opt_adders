...
class prefix_tree(graph):
                ...
                for b in range(self.w):
                    if ctr > 0:
                        ...
                        if a == lg(b) + 1:
                            ...
                            node_color = "black"
                        self.add_node(
                            node(b, a, self.node_defs[node_color]),
                            pre=self[b + ctr - 1, a - 1],
                        )
                        ctr -= 1
            self.dna = "sklansky"
            ...
            for a in range(1, self.w):
                for b in range(self.w):
                    if b != a:
                        ...
                        self.add_node(
                            node(b, a, self.node_defs["grey"]), pre=self[b - 1, a - 1]
                        )
            self.dna = "ripple"
    ...
    def add_node(self, n, pre=None):
        ...
        if pre is not None and pre.m not in self.node_defs.values():
            raise ValueError(
                "provided predecessor node type must be defined for this tree"
            )
    ...
    def walk_downstream(self, n, fun=lambda x: x):
        ...
        # Iterate
        if bot is not None:
            self.walk_downstream(bot, fun)
        for x in post:
            self.walk_downstream(x, fun)
    ...
    def _add_top(self, n, pre=None):
        ...
        for name, bits, num_diag in modules[n.m]["ins"]:
            ...
            try:
                name_m, bits_m = next(
                    x for x in modules[top.m]["outs"] if x[0] == verso_pin(name)
                )
    ...
    def _add_pre(self, n, pre):
        ...
        for name, bits, num_diag in modules[n.m]["ins"]:
            ...
            try:
                match = next(
                    x for x in modules[pre.m]["outs"] if x[0] == verso_pin(name)
                )
            # If not, just leave it unconnected and hope for the best?..
            except StopIteration:
                continue
    ...
    def r_right(self, n, c=[]):
        ...
        right = self.right(n)
        if (
            (node._exists(right) and not node._isbuf(right))
            or right is None
            or right in c
        ):
            return right
    ...
    def _valid_tops(self, a, a_bits, b, x, c=[], d=[], path=[], result=(None, None)):
        ...
        if not node._in_tree(x):
            ...
            if len(path) == 0:
                return None, None
            # Otherwise fork up
            return self._valid_tops(
                a, a_bits, b, self.top(path[-1]), c, d, path[:-1], result
            )
        ...
        if pre is None and x not in c:
            ...
            for y in possi:
                ...
                tmp = self._valid_tops(
                    a, a_bits, b, y, c + [x], d + [y], path + [x], result
                )
    ...
    def _checkLF(self, x, y=None):
        ...
        if not isinstance(x, int) or (y is not None and not isinstance(y, int)):
            raise TypeError(
                "x,y values provided to the internal-use-only check function are invalid!"
            )
    ...
    def _checkFL(self, x, y=None):
        ...
        for x in reversed(self.node_list[y][pre.x : x]):
            top = self.top(x)
            # ∃ b s.t pre(a)=pre(b),
            if self.pre(x) == pre or (top == pre and node._isbuf(x)):
                ...
                for y in self.post(a):
                    topy = self.top(y)
                    if not self._is_pg_subset((topy, x), (topy, a)) and node._exists(
                        topy
                    ):
                        flag = False
                        break
        ...
        if b is not None and (
            not node._exists(bot)
            or modules[bot.m]["type"] == "post"
            or node._isbuf(bot)
        ):
            return (a, b)
    ...
    def clean(self):
        ...
        if self.is_idem:
            while self.reduce_idem() or self.compact():
                pass
        else:
            while self.compact():
                pass
    ...
    def reduce_idem(self):
        ...
        for a in self:
            ...
            if node._isbuf(a):
                ...
                if len(self.post(a)) > 1 and not in_the_way:
                    continue
                if len(self.post(a)) == 1 and not black_under:
                    continue
            ...
            # and has no posts?
                ...
                not node._isbuf(rbot)
                and len(self.post(a)) == 0
                and self._is_pg_subset((self.pre(rbot), rtop), (pre, rtop))
            ):
                modified = a
    ...
    def png(self, fname="out.png"):
        ...
        def add_invis(orig_pos, x_dir=1, y_dir=1):
            ...
            if len(pg.get_node(wrap(new_pos))) == 0:
                pg.add_node(new_n)
            return new_pos
        ...
        for row in self.node_list:
            for n in row:
                ...
                if (
                    n.x == 0
                    or n.y in [0, len(self.node_list) - 1]
                    or not node._exists(n)
                ):
                    continue
                ...
                rights = iter(
                    [
                        ...
                        if self.pre(x) == pre
                        and x.x < n.x
                        and (node._exists(x) and not node._isbuf(x))
                    ]
                )
