...
class ExpressionNode:
    ...
    def __len__(self):
        """Returns the depth of the subtree generated by this node"""
    ...
    def is_complete(self):
        """Checks if the subtree generated by this node is complete"""
        # Avoid repeated depth calculations (to some degree)
        c_depths = [len(c) for c in self]
        depth = 1 + max(c_depths)
        # First confirm the subtree has minimal depth
        if 1<<(depth-1) >= bin(self.leafs).count("1"):
            return False
        # Confirm that the children have depths consistent with completeness
        prev_d = depth - 1
        for d in c_depths:
            if d > prev_d:
                return False
        ...
        # Fall-through case if all children are proper and have the same depth
        return True

    def is_proper(self):
        """Checks if the subtree generated by this node is proper"""
        return 1<<len(self) == bin(self.leafs).count('1')

    def least_significant_descendant(self):
        """Finds the least significant descendant of this node"""
        ...
        if idx == -1:
            return self
        return self[idx].least_significant_descendant()

    def most_significant_descendant(self):
        """Finds the most significant descendant of this node"""
        idx = 0
        for c in self:
            if self[idx] is None:
                idx += 1
        ...
        if idx == len(self.children):
            return self
        return self[idx].most_significant_descendant()
